import React, {
  ...
  const TABS: Record<TabType, { label: string; component: React.LazyExoticComponent<React.FC> }> = {
    first: {
      label: "First Heavy Component",
      component: lazy(() => import("./components/HeavyComponent")),
    },
    second: {
      label: "Second Heavy Component",
      component: lazy(() => import("./components/Reports")),
    },
    third: {
      label: "Third Heavy Component",
  ...
  const selectFilteredOrders = (state: State) => {
    return state.orders.filter(order => {
      const matchesFilter =
        state.filterOrder === "ALL" || order.status === state.filterOrder;
      const matchesQuery =
        order.customerName.toLowerCase().includes(state.queryOrder.toLowerCase());
  ...
  const useTabs = () => {
    ...
    const renderActiveComponent = () => {
      ...
      if (!config) return <div>Tab not found</div>;
  ...
  const useTodos = () => {
    ...
    return { todos, query: state.queryTodo, setQuery, addTodo, toggleTodo, deleteTodo };
  };
  ...
  const useOrders = () => {
    ...
    const stats = useMemo(() => selectOrderStats(state), [
      state.orders,
      state.filterOrder,
      state.queryOrder,
    ]);
    ...
    return {
      ...
      stats,
      setQuery: (q: string) =>
        dispatch({ type: "SET_QUERY_ORDER", payload: q }),
      setFilter: (f: Status | "ALL") =>
        dispatch({ type: "SET_FILTER_ORDER", payload: f }),
      setCount: (c: number) =>
  ...
  const ComponentsLoading: React.FC = () => {
    const { setActiveTab, renderActiveComponent } = useTabs();
  ...
  const ComponentTodos: React.FC = () => {
    const { todos, query, setQuery, addTodo, toggleTodo, deleteTodo } = useTodos();
...
//     return { todos, query: state.todoQuery, setQuery, addTodo, toggleTodo, deleteTodo };
//   };
...
//       stats,
//       setQuery: (q: string) =>
//         dispatch({ type: "SET_ORDER_QUERY", payload: q }),
//       setFilter: (f: Status | "ALL") =>
